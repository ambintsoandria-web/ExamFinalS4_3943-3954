...
<div class="metric-row"><div class="metric-card"><span><i class="bi bi-people"></i> Clients actifs</span><strong><?= (int) $nombreClients ?></strong></div><div class="metric-card"><span><i class="bi bi-arrow-left-right"></i> Opérations</span><strong><?= array_sum(array_map('intval', array_column($stats, 'total'))) ?></strong></div><div class="metric-card"><span><i class="bi bi-cash-coin"></i> Volume total</span><strong><?= number_format(array_sum(array_map('floatval', array_column($stats, 'montant'))), 0, ',', ' ') ?> <small>Ar</small></strong></div></div>
<section class="charts-grid">
    <div class="chart-card"><div class="chart-head"><div><span>Vue globale</span><h2>Opérations par type</h2></div><i class="bi bi-bar-chart"></i></div><div class="chart-box"><canvas id="operationsChart"></canvas></div></div>
    <div class="chart-card"><div class="chart-head"><div><span>7 derniers jours actifs</span><h2>Activité récente</h2></div><i class="bi bi-activity"></i></div><div class="chart-box"><canvas id="activityChart"></canvas></div></div>
</section>
